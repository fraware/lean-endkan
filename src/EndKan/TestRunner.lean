...
def createTestSuites (config : TestConfig) (testTypes : List String) : List TestSuite :=
  ...
  let suiteNameMatches (requested suiteName : String) : Bool :=
    (requested == "unit" && suiteName == "Unit Tests")
    || (requested == "integration" && suiteName == "Integration Tests")
    || (requested == "e2e" && suiteName == "End-to-End Tests")
    || (requested == "performance" && suiteName == "Performance Tests")
    || (requested == "perf" && suiteName == "Performance Tests")

  allSuites.filter fun suite =>
    requestedTypes.any fun testType => suiteNameMatches testType suite.name
